...
ManifestPlugin.prototype.existNameInManifest = function (name, manifest) {
  // is not in key nor in value in manifest collection
  return (typeof manifest[name] !== 'undefined') || (_.values(manifest).indexOf(name) !== -1);
};
  ...
  compiler.plugin('emit', function (compilation, compileCallback) {
    ...
    _.merge(manifest, stats.assets.reduce(function (memo, asset) {
      ...
      if (name) {
        ...
        if (!this.existNameInManifest(asset.name, manifest)) {
          memo[asset.name] = asset.name;
        }
      }
      return memo;
    }.bind(this), {}));
