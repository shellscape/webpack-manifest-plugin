...
ManifestPlugin.prototype.apply = function(compiler) {
  ...
  var emit = function(compilation, compileCallback) {
    ...
    var evaluateAllCompilation = function (compilation, files) {
      if (compilation.children.length) {
        compilation.children.forEach(function (compilation) {
          files = evaluateAllCompilation(compilation, files);
        })
      }

      return compilation.chunks.reduce(function (files, chunk) {
        return chunk.files.reduce(function (files, path) {
          ...
          // some situation compilation assets do not have the chunk.
          // if it does not exist, it will be excluded.
          if (!compilation.assets.hasOwnProperty(path)) {
            return files;
          }
          ...
          return files.concat({
            ...
            isAsset: false,
            isModuleAsset: false,
            isChild: !!compilation.compiler.parentCompilation
          });
        }.bind(this), files);
      }.bind(this), files);
    }.bind(this);

    var files = evaluateAllCompilation(compilation, []);
    var entrySet = new Set(files.map(file => file.path));

    // module assets don't show up in assetsByChunkName.
    // we're getting them this way;
    files = stats.assets.filter(function (asset) {
      return !entrySet.has(asset.name);
