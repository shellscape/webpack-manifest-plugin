...
function applyDefaultWebpackConfig(webpackOpts) {
  const defaults = isWebpackVersionGte(4)
    ? {
        optimization: {
          chunkIds: 'natural'
        }
...
describe('ManifestPlugin using real fs', () => {
      ...
      webpackCompile(
        ...
        () => {
          const manifest = fse.readJsonSync(
            path.join(__dirname, 'output/single-file/manifest.json')
          );
      ...
      webpackCompile(
        ...
        (stats) => {
          ...
          const manifest1 = fse.readJsonSync(
            path.join(__dirname, 'output/single-file/manifest1.json')
          );
          ...
          const manifest2 = fse.readJsonSync(
            path.join(__dirname, 'output/single-file/manifest2.json')
          );
      ...
      webpackCompile(
        {
          ...
          entry: './fixtures/file.js',
          plugins: [new ManifestPlugin(), testPlugin]
        },
        {},
        () => {
    ...
    it('outputs a manifest of one file', (done) => {
      compiler = webpackWatch(
        ...
        (stats) => {
          ...
          fse.outputFileSync(
            path.join(__dirname, 'output/watch-mode/index.js'),
            "console.log('v2')"
          );
        }
      );
  ...
  describe('multiple manifest', () => {
    ...
    it('should produce two seperate manifests', (done) => {
      webpackCompile(
        [
          {
            context: __dirname,
