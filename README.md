...
* `seed`: In [multi-compiler mode](https://github.com/webpack/webpack/tree/master/examples/multi-compiler) webpack will overwrite the manifest on each compilation. Passing a shared `{}` as the `seed` option into each compilation's ManifestPlugin will combine the manifest between compilations. Also use for [custom keys](https://developer.mozilla.org/en-US/Add-ons/WebExtensions/manifest.json) in your manifest.
